"""레이어별 풀해상도 렌더링(바운딩 박스 단위) — S6 세션 1.
...
def render_manifest(manifest: Manifest) -> RenderResult:
    """매니페스트 전체를 CMYK PSD로 렌더·저장 — 항목 순서 = 레이어 순서(아래→위).
    ...
    layers = [_render_item(item, dpi, index) for index, item in enumerate(manifest.items)]

    manifest.output_path.parent.mkdir(parents=True, exist_ok=True)
    psd_writer.write_psd(manifest.output_path, (width_px, height_px), layers)
    return RenderResult(
...
def _render_item(item: ManifestItem, dpi: int, index: int) -> LayerSpec:
    """항목 1개를 레이어로 렌더 — 디스크 로드→리사이즈→회전→알파 분리→CMYK."""
    ...
    cmyk = color.rgb_to_cmyk(placed.convert("RGB"))
    ...
    return LayerSpec(
        image=cmyk,
        name=_layer_name(item.src, index),
        top=top,
        left=left,
